...
# ➕ Preencher combinações faltantes
...
# 🧼 Limpeza final
filtered_df['MÊS'] = pd.Categorical(filtered_df['MÊS'], categories=ordered_months, ordered=True)
filtered_df[quantity_col] = pd.to_numeric(filtered_df[quantity_col], errors='coerce').fillna(0)
if 'PM' in filtered_df.columns:
    filtered_df['PM'] = pd.to_numeric(filtered_df['PM'], errors='coerce').fillna(0)
...
# 📈 Gráfico de linhas — Quantidade
pivot_data = filtered_df.groupby(['ANO', 'MÊS'])[quantity_col].sum().reset_index()
