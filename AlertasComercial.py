...
# =============================================
# CONFIG & ESTILO (MÍNIMO E EFICIENTE)
# =============================================
...
<style>
    ...
    [data-testid="metric-value"] {font-size:2.2rem !important; font-weight:800 !important}
</style>
""", unsafe_allow_html=True)

# =============================================
# DADOS (OTIMIZADO: 1 LINHA POR VENDA)
# =============================================
...
def load_data():
    ...
    df.rename(columns=lambda x: x.replace('v. líquido', 'v_liquido').replace('qtd.', 'qtd'), inplace=True)
    
    # Valor Líquido (europeu → float)
    ...
    df = df[(df['mes'].between(1,12)) & (df['qtd'] > 0)]
...
with st.sidebar:
    ...
    def filter_options(data, ano, com, cli):
        d = data.copy()
        if ano != "Todos": d = d[d['ano'] == int(ano)]
        if com != "Todos": d = d[d['comercial'].astype(str) == str(com)]
        if cli != "Todos": d = d[d['cliente'].astype(str) == str(cli)]
        return (sorted(d['ano'].unique().astype(int)),
                sorted(d['comercial'].unique()),
                sorted(d['cliente'].unique()),
                sorted(d.get('categoria', pd.Series()).dropna().unique()))

    anos = sorted(df['ano'].unique().astype(int))
    ano = st.selectbox("Ano", ["Todos"] + anos)
    coms = filter_options(df, ano, "Todos", "Todos")[1]
    comercial = st.selectbox("Comercial", ["Todos"] + coms)
    cls = filter_options(df, ano, comercial, "Todos")[2]
    cliente = st.selectbox("Cliente", ["Todos"] + cls)
    cats = filter_options(df, ano, comercial, cliente)[3]
...
# FUNÇÕES ÚTEIS
# =============================================
fmt = lambda x, unit: f"{x:,.0f} {unit}" if pd.notna(x) else f"0 {unit}"
...
if page == "Visão Geral":
    ...
    st.metric("Valor", fmt(dfc['v_liquido'].sum(), "€"), delta=None)
    ...
    if len(anos) < 2: st.info("Apenas um ano.")
        ...
        with c1:
            ...
            st.metric(f"Valor {a1}", fmt(d1['v_liquido'].sum(), "€"))
        with c2:
            st.metric(f"Qtd {a2}", fmt(d2['qtd'].sum(), "kg"))
            st.metric(f"Valor {a2}", fmt(d2['v_liquido'].sum(), "€"))

# Exportar dados
if st.sidebar.button("Exportar Dados"):
    st.download_button("Baixar", export_excel(data), "vendas.xlsx")
