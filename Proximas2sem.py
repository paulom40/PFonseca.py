...
names = ["Paulo Costa", "Maria Silva", "João Costa"]
usernames = ["paulo", "maria", "joao"]
passwords = stauth.Hasher(["teste", "senha456", "senha789"]).generate()

authenticator = stauth.Authenticate(
    names, usernames, passwords,
    "vencimentos_app", "abcdef", cookie_expiry_days=3650
)

name, authentication_status, username = authenticator.login("🔐 Login Comercial", "main")
...
if authentication_status is False:
    ...
    # --- FILTRAR POR COMERCIAL LOGADO ---
    if 'Comercial' in df.columns:
        df = df[df['Comercial'] == name]
